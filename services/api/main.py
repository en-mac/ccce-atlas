...
async def detailed_health_check():
    """
    Comprehensive health check with smoke tests for all critical components.
    ...
    # 3. Parcel tile system check (sample tile query)
    try:
        async with pool.acquire() as conn:
            # Test a downtown Corpus Christi tile
            rows = await conn.fetch("""
                SELECT COUNT(*) as count
                FROM parcels
                WHERE geom && ST_MakeEnvelope(-97.4, 27.79, -97.39, 27.81, 4326)
                LIMIT 100
            """)
            count = rows[0]["count"]
            results["checks"]["parcel_tiles"] = {
                "status": "healthy",
                "sample_tile_parcels": min(count, 100),
                "message": f"Parcel tile query returned {count} parcels"
            }
    except Exception as e:
        results["status"] = "degraded"
        results["checks"]["parcel_tiles"] = {
