#!/bin/bash
#
# Push-Based CI/CD Deployment Script for EC2
# This script runs on EC2 after GitHub Actions has pushed files via rsync
# NO git operations - code is already on disk from rsync
#

set -e

echo "======================================"
echo "Starting push-based deployment..."
echo "======================================"

# Navigate to project directory (should exist from rsync)
cd ~/ccce-atlas || {
  echo "❌ Error: ~/ccce-atlas directory not found!"
  echo "GitHub Actions should have rsync'd files before running this script"
  exit 1
}

# Clean any generated files (excluding .git directory if it exists)
...
# Pull/rebuild images (use cache for faster builds)
